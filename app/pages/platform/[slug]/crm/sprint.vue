...
const createSprint = async () => {
  ...
  try {
    await crmStore.createSprint(slug.value, { ...form, name: form.name.trim() })
      ...
      <v-dialog v-model="showCreateDialog" max-width="620">
        <v-card>
          ...
          <v-card-text>
            ...
            <v-text-field v-model="form.startDate" label="Start date (ISO)" />
            <v-text-field v-model="form.endDate" label="End date (ISO)" />
          </v-card-text>
          <v-card-actions>
            <v-spacer />
