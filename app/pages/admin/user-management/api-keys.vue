    ...
    >
      ...
      <template #item.actions="{ item }">
        <div class="d-flex justify-end flex-nowrap ga-1 py-1">
          <v-btn size="x-small" variant="text" icon="mdi-eye" :aria-label="t('admin.apiKeys.aria.show', { id: item.id })" @click="showEntity(item.id)" />
          ...
          <v-btn size="x-small" variant="text" color="error" icon="mdi-delete" :aria-label="t('admin.apiKeys.aria.delete', { id: item.id })" @click="openDeleteDialog(item.id)" />
        </div>
      </template>
    </UiDataTable>
